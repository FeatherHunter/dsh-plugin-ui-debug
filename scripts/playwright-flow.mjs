// playwright-flow.mjs — 最佳实践全流程：激活会话 → 点可接 → 面板开 → 拖拽双程采样
import { chromium } from 'playwright-core'
import { mkdirSync } from 'node:fs'

const DSH = 'http://127.0.0.1:59519'
const OUT = 'D:/dsh-plugin/dsh-mattpocock-skills-deck/.tmp-screenshots/pwflow'
...
try {
  ...
  const sess = await page.evaluate(() => {
    ...
    for (const k of keys) {
      const all = Array.from(document.querySelectorAll('*')).filter((el) => {
        ...
        return t === k && el.children.length <= 3
      })
      const leaf = all.sort((a, b) => a.getBoundingClientRect().width - b.getBoundingClientRect().width)[0]
      if (leaf) { const r = leaf.getBoundingClientRect(); return { x: r.x + r.width / 2, y: r.y + r.height / 2, text: k, tag: leaf.tagName, cls: String(leaf.className || '').slice(0, 30) } }
    }
    return null
  })
  ...
  console.log('after session click ::', short(await tabsState(page)))
  ...
  console.log('after 可接 click ::', short(await tabsState(page)))

  // 3) 扫描分隔条命中点（nArs4W_panelResize）
  ...
  if (hit) {
    ...
    console.log('!! 分隔条不可命中；面板是否出现：', short(await tabsState(page)))
  }
} finally { /* 保留窗口 */ }
console.log('PW-FLOW-DONE')
await new Promise(() => {})
