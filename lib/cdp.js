        ...
        try {
            ...
            if (!isHeadless) {
                if (!wantForeground) {
                    ...
                    try {
                        const win = await session.send('Browser.getWindowForTarget', tab.id ? { targetId: tab.id } : {});
                        ...
                        if (typeof windowId === 'number') {
                            ...
                            const wid2 = win2?.windowId;
                            if (typeof wid2 === 'number')
                                await session.send('Browser.setWindowBounds', { windowId: wid2, bounds: { windowState: 'minimized' } });
                        }
                    }
                    ...
                    try {
                        ...
                        try {
                            await session.evaluate(`window.dispatchEvent(new FocusEvent('focus')); document.dispatchEvent(new Event('visibilitychange'))`);
                        }
                        catch { /* ignore */ }
                    }
                }
                    ...
                    try {
                        ...
                        if (typeof windowId === 'number') {
                            ...
                            const wid2 = win2?.windowId;
                            if (typeof wid2 === 'number')
                                await session.send('Browser.setWindowBounds', { windowId: wid2, bounds: { windowState: 'maximized' } });
                        }
                    }
