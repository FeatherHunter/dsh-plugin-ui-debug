...
const SKILL_DESC = '[DSH 插件开发] 用真实 Chrome (Playwright) 对 DSH 插件 UI 做闭环调试：UI查看→UI测试→UI验证→问题解决。' +
...
function assertHttpUrl(url) {
    ...
    if (!/^https?:\/\//i.test(s))
        throw new Error(`无效 URL（仅 http/https）: ${JSON.stringify(url)}`);
    return s;
}
function asInt(value, name, fallback) {
...
async function runDrive(session, actions, outDir) {
    const lines = [];
    const shots = [];
    const evalResults = [];
    let index = 0;
    const coordOf = async (a, key) => {
        const sel = typeof a[`${key}Selector`] === 'string' ? a[`${key}Selector`] : undefined;
        if (sel)
            return session.elementCenter(sel);
        const x = a[`${key}X`];
        const y = a[`${key}Y`];
        ...
        try {
            switch (type) {
                ...
                case 'eval': {
                    ...
                    if (!expr)
                        throw new Error('eval 需要 expression');
                ...
                case 'click': {
                    const sel = typeof a.selector === 'string' ? a.selector : undefined;
                    if (sel) {
                ...
                case 'type': {
                    ...
                    if (!sel || text === '')
                        throw new Error('type 需要 selector 和 text');
                ...
                case 'scroll': {
                    const to = a.to;
                    await session.scroll(to === 'top' || to === 'bottom'
                        ? { to }
                        : {
                ...
                case 'drag': {
                    ...
                    if (!from || !to)
                        throw new Error('drag 需要 from(selector 或 fromX/fromY) 与 to(toX/toY)');
...
export function apply(ctx) {
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        name: 'ui_shot',
        description: `截图一个 http/https 页面（默认是运行中的 DSH GUI 自身地址），把 PNG 存到磁盘并返回文件路径。` +
        ...
        execute: async (args) => {
            ...
            const dir = typeof args.out === 'string' && args.out.trim() !== ''
                ...
                : defaultOutDir();
            const maximized = asBool(args.maximized, true);
            const headless = asBool(args.headless, true);
            const foreground = asBool(args.foreground, false);
            const width = typeof args.width === 'number' || typeof args.width === 'string'
                ? asInt(args.width, 'width', maximized ? 1920 : 1440)
                : (maximized ? 1920 : 1440);
            const height = typeof args.height === 'number' || typeof args.height === 'string'
                ? asInt(args.height, 'height', maximized ? 1080 : 900)
                : (maximized ? 1080 : 900);
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        name: 'ui_drive',
        description: `在一个无头浏览器会话里按动作脚本驱动页面（导航/点击/输入/按键/滚动/拖拽 + 分步截图 + JS 求值），` +
        ...
        parameters: {
            ...
            actions: {
                ...
                description: 'JSON 数组字符串。支持的动作（type）与字段：\n' +
                    '- {"type":"navigate","url":"…"}\n' +
                    '- {"type":"wait","ms":500}\n' +
                    '- {"type":"waitReady"}\n' +
